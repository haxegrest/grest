...
interface TransferConfigs {
	...
	**/
	...
	var versionInfo : String; }, body:grest.bigquerydatatransfer.v1.types.TransferConfig):grest.bigquerydatatransfer.v1.types.TransferConfig;
	@:sub("/")
	var runs : grest.bigquerydatatransfer.v1.api.projects.locations.transferConfigs.Runs;
	/**
		Creates transfer runs for a time range [start_time, end_time]. For each date - or whatever granularity the data source supports - in the range, one transfer run is created. Note that runs are created per UTC time in the time range. DEPRECATED: use StartManualTransferRuns instead.
	**/
	@:post("/v1/$parent")
	function scheduleRuns(parent:grest.bigquerydatatransfer.v1.types.Api_bigquerydatatransfer_projects_locations_transferConfigs_scheduleRuns_parent_Command, body:grest.bigquerydatatransfer.v1.types.ScheduleTransferRunsRequest):grest.bigquerydatatransfer.v1.types.ScheduleTransferRunsResponse;
	/**
		Start manual transfer runs to be executed now with schedule_time equal to current time. The transfer runs can be created for a time range where the run_time is between start_time (inclusive) and end_time (exclusive), or for a specific run_time.
	**/
	@:post("/v1/$parent")
	function startManualRuns(parent:grest.bigquerydatatransfer.v1.types.Api_bigquerydatatransfer_projects_locations_transferConfigs_startManualRuns_parent_Command, body:grest.bigquerydatatransfer.v1.types.StartManualTransferRunsRequest):grest.bigquerydatatransfer.v1.types.StartManualTransferRunsResponse;
}